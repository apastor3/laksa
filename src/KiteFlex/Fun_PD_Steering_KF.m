function PD = Fun_PD_Steering_KF
...
PD.Env.Vw     = 12;               % Wind Velocity                     (m/s)
...
PD.Bridle.delta = 60*pi/180;      % delta                              (rad)
...
% Inertia of each rotor
PD.Gen.m        =  0.0; % kg, Mass of the three blade
PD.Gen.L        =  0.0; % m, length of a blade


% Control parameters
PD.Control.Type      = 6;            % See Fun_Control  
                                 
PD.Control.T0        = 20;           % Control period 

PD.Control.eps_lt    = 0.;           % lt = lt0*(1+eps_lt*sin(ome_lt*t)) or % lt = lt0*(1+eps_lt*t)  
...
PD.Control.eps_eta   = -17.5*pi/180; % eta   = eps_eta*sin(ome_eta*t))
PD.Control.ome_eta   =  0.3;         %
